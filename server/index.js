...
app.post("/get-latest-magazine", async (req, res) => {
  try {
    const latestBooks = await Magazine.find({})
      .sort({ createdAt: -1 }) // Sort by createdAt field in descending order (most recent first)
      .limit(3); // Limit the result to 5 documents
    res.json({ data: latestBooks });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Internal Server Error" });
...
app.post("/edit-chp", async (req, res) => {
  try {
    ...
    if (!bookInfo) {
      return res.send({ message: "Chapter doesn't exists!" });
    }
    await bookInfo.save();
    return res.send({ message: "Chapter edited Successfully!", status: "ok" });
  } catch (error) {
    console.log(error);
    res.status(500).send({ error: "Internal Server Error" });
  }
...
app.post("/del-chp", async (req, res) => {
  try {
    ...
    return res.send({ message: "Chapter deleted Successfully!", status: "ok" });
  } catch (error) {
    console.log(error);
    res.status(500).send({ error: "Internal Server Error" });
  }
...
app.post("/store-payment-details", async (req, res) => {
  ...
  try {
    ...
    const logoPath =
      "../client/src/assets/website/more_semi_transparent_image.png"; // Adjust this to the path of your logo
